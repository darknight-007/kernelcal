...
- Runs from this directory so ``kernelcal`` resolves (same pattern as
  ``bf_kernelcal_demo.py``).

Usage
-----
  cd manuscripts/software-kernelcal-deepgis-integration
  PYTHONPATH=/path/to/RivGraph/_deps:$PYTHONPATH MPLBACKEND=Agg \\
    python3 rivgraph_kernelcal_bridge.py

  python3 rivgraph_kernelcal_bridge.py --mask /path/to/mask.tif --exit-sides NS

  # write PNGs under results_folder / plots (or --plot-dir)
  python3 rivgraph_kernelcal_bridge.py --plot --rivgraph-repo /path/to/rivgraph

  # also save Laplacian eigenvector maps + heatmap (modes 1..K; 1 = Fiedler)
  python3 rivgraph_kernelcal_bridge.py --plot --plot-eigenmodes 6 --rivgraph-repo /path/to/rivgraph

  # Colville delta (pruned network) -> kernelcal
  python3 rivgraph_kernelcal_bridge.py --delta --plot --rivgraph-repo /path/to/rivgraph
"""
