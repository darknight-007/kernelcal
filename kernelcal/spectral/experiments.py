...
# ---------------------------------------------------------------------------
# Shared style helpers (N-agnostic)
# ---------------------------------------------------------------------------
...
def _draw_topology(ax, coords: dict[int, tuple[float, float]], edges, choke_edge, title: str):
    """Draw a simple node-edge topology schematic."""
    for (i, j) in edges:
        x1, y1 = coords[i]
        x2, y2 = coords[j]
...
def _save_topology_schematics(output_dir: Path) -> None:
    """Save schematic figure for river and trunk-roots topologies."""
    # River-channel schematic
    river_coords: dict[int, tuple[float, float]] = {i: (i, 0.0) for i in range(10)}
    river_coords.update(
        {
            10: (2.2, 1.2),
            11: (4.0, 1.3),
    ...
    river_edges = [(i, i + 1) for i in range(9)] + [(2, 10), (4, 11), (4, 12), (6, 13), (7, 14), (8, 15)]

    # Trunk+roots schematic
    trunk_coords: dict[int, tuple[float, float]] = {i: (0.0, i) for i in range(8)}
    trunk_coords.update(
        {
            8: (-1.0, -0.2),
            9: (-1.3, 0.5),
    ...
    trunk_edges = [(i, i + 1) for i in range(7)] + [
        (0, 8), (0, 9), (0, 10), (1, 11), (1, 12), (6, 13), (6, 14), (7, 15), (7, 16)
    ]
    ...
    fig.tight_layout()
    _save(fig, output_dir, "exp7_topologies.pdf")
