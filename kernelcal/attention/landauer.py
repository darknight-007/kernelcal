...
# ── Figure generation ────────────────────────────────────────────────────
#
# Plotting lives in ``kernelcal.attention.landauer_plots`` so that this
# module can be imported on headless installs without pulling matplotlib.
# The underscore names are preserved as re-exports for back-compat
# (``add_wall_power.py`` imports ``_generate_landauer_figures*`` from here).

from .landauer_plots import (  # noqa: E402, F401  (re-export)
